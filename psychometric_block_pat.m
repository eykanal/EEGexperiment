function [ST, ST_time, RT, ER, RDir, PiDirbl, SubScore, score, premie_t, premie_d,coherenceVec] = psychometric_block (D, shape, coher,blockdur, dotsIdx,targetIdx,blackTargetIdx,textIdx_Score,lkey, rkey,  PI, feedback, salary,wdwPtr,pahandle_correct, pahandle_antic, daq)
...
for n  = 1:length(coherenceVec)
  ...
  if ST_bool
    [RT(n), ER(n), RDir(n), score, premie_t{n}, ...
     premie_d{n},ST_time(n)] = trial_fixedview (dotsIdx, targetIdx,blackTargetIdx,textIdx_Score, ppd_, 0, rkey, RSI_vector, shape,score, ntrial, money, salary,wdwPtr,1000,pahandle_correct, pahandle_antic, daq);
    ST(n) = 'R';     
    % Now store trial-by-trial scores:
    SubScore(n) = score;
  else
    [RT(n), ER(n), RDir(n), score, premie_t{n}, ...
     premie_d{n},ST_time(n)] = trial_fixedview (dotsIdx, targetIdx,blackTargetIdx,textIdx_Score, ppd_, 180, lkey, RSI_vector, shape,score, ntrial, money, salary, wdwPtr,1000,pahandle_correct, pahandle_antic, daq);
    ST(n) = 'L';
    % Now store trial-by-trial scores:
    SubScore(n) = score;
  end 
...
% 3/10/09: Pat had to comment this out, because it can fail with small
% numbers of trials:
