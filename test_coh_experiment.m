% function acc = test_coh_experiment (subject,coh)
% this experiment runs a single block for a desired coherence to
...
if ~exist('coh','var')
  fprintf('Need coherence as second input argument\n');
  return
end
...
SessionsCompleted = 0;
filename = sprintf ('subject%d_testses%d.mat', subject, SessionsCompleted + 1);
...
textIdx_Run = rAdd('dXtext', length(run_str), 'visible', false, 'x', x_pos_run, 'y',{0 -5}, 'size', fontsize, 'font', 'Courier', 'color', contrast_factor*[1 1 1]*255, 'string', run_str);
...
for bl = 1:numblocks
  ...
  [STbl, irrelDirbl,ST_timebl, arrow_onsetbl, arrow_durationbl, arrow_offsetbl,RTbl, ERbl, RDirbl, PiDirbl, score, premie_t,premie_d] = ...
      block (RSI,5,coh, 'd',blockdur, dotsIdx,targetIdx,blackTargetIdx,textIdx_Score,ppd_,lkey, hkey, money,salary, wdwPtr,pahandle_correct, pahandle_antic, daq,contrast_factor);
  ...
  save (filename, 'ST', 'ST_time', 'RT', 'ER', 'RDir', 'blocknum', 'trialnum','D', 'PI', 'PiDir', 'money','coh','numblocks');
