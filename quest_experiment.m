function money = quest_experiment (subject)
% function money = quest_experiment( subject )
%
% Find the coherence for 75% correct for this participant
...
blockdur = 100;		%number of trials in the quest blocks

numblocks = 1;
% checking existence of the datafile
SessionsCompleted = 0;
filename = sprintf ('subject%d_questses%d.mat', subject, SessionsCompleted + 1);
...
money = 0;
tmp_coherence = 30; % coherence is irrelevant because it will be
...
qSet = []; % set of quest distributions
...
for bl = 1:numblocks
  ...
  [STbl, ST_timebl, RTbl, ERbl, RDirbl, PiDirbl, SubScore, score, premie_t,premie_d,cohMean(bl),cohStd(bl),q] = quest_block (RSI,5,tmp_coherence, blockdur, dotsIdx,targetIdx,blackTargetIdx,textIdx_Score,lkey, hkey, PIconst, feedback_flags(bl), salary, wdwPtr,pahandle_correct, pahandle_antic, daq);
  ...
  qSet = [qSet q]; 
  fprintf('mean coherence (std): %.02f (%.02f)\n',cohMean(bl),cohStd(bl));
  save (filename);
end

% now run a normal block with the obtained coherence to check
% whether it indeed produces the desired accuracy
tmp_coherence = mean(cohMean);
bl = bl+1;
resting_arrow (1, money,'d' );
[STbl, irrelDir,ST_timebl, arrow_onset,arrow_duration,arrow_offset,RTbl, ERbl, RDirbl, PiDirbl, score, premie_t,premie_d] = ...
    block(RSI,5,tmp_coherence, 'd',blockdur, dotsIdx, targetIdx, blackTargetIdx, textIdx_Score, ppd_, lkey, hkey, money, salary,wdwPtr, pahandle_correct, pahandle_antic, daq,contrast_factor);
ib = length(RT) + 1;		%index of the begining of inserted block
ie = length(RT) + length(RTbl);	%index of the end of inserted block
ST(ib:ie) = STbl;
ST_time(ib:ie) = ST_timebl;
...
thankyou (money, 10);
save (filename, 'ST', 'ST_time', 'RT', 'ER', 'RDir', 'blocknum', 'trialnum', ...
	'D', 'PI', 'PiDir', 'SubScore', 'money', 'cohMean','cohStd','qSet');
