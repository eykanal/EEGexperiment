function dots_ft_avg(subj, sess, run, dataset)
...
for aveTime = 1:length(aveTimes)
    for aveParam = 1:length(aveParams)
        ...
        for aveParamValue = 1:length(aveParams{aveParam}{2})
            ...
            % Load data        
            load([subj_data '-preprocessed-'  char(aveTimes(aveTime)) '-' num2str(aveParams{aveParam}{1}) '-' num2str(aveParams{aveParam}{2}(aveParamValue)) '.mat']);
        ...
        % ## TODO - Create a difference file between the conditions
